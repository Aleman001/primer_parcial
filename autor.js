Vue.component('autor'), {
    ...
    methods:{
        ...
        guardarDocente(){
            ...
            let store = this.abrirStore("tblautor", 'readwrite');
        ...
        listarAutor(){
            let store = this.abrirStore('tblautor', 'readonly'),
                data = store.getAll();
            data.onsuccess = resp=>{
                this.autor = data.result
                    .filter(autor=>autor.nombre.toLowerCase().indexOf(this.buscar.toLowerCase())>-1 || 
                        autor.codigo.indexOf(this.buscar)>-1);
            }
        }
    }
};
        ...
        <div class="row">
            <div class="col-12 col-md-6">
                <div class="card border-primary">
                    <div class="card-header bg-primary text-white">REGISTRO DE AUTOR</div>
                    <div class="card-body">
                        <form id="frmAutor" @submit.prevent="guardarAutor" @reset.prevent="nuevoAutor()">
                            <div class="row p-1">
                                <div class="col-3 col-md-2">CODIGO:</div>
                                <div class="col-9 col-md-3">
                                    <input required pattern="[0-9]{3}" class="form-control" type="text" v-model="autor.codigo">
                                </div>
                            </div>
                            <div class="row p-1">
                                <div class="col-3 col-md-2">NOMBRE:</div>
                                <div class="col-9 col-md-6">
                                    <input required pattern="[a-zA-Z ]{3,65}" class="form-control" type="text" v-model="autor.nombre">
                                </div>
                            </div>
                            <div class="row p-1">
                        ...
                        <form>
                            <table class="table table-dark table-hover">
                                <thead>
                                    <tr>
                                        <th>BUSCAR:</th>
                                        <th colspan="2"><input type="text" @keyup="listarAutor()" v-model="buscar" 
                                ...
                                <tbody>
                                    <tr v-for="docente in autor" @click='modificarAutor(Autor)' :key="autor.idAutor">
                                        <td>{{Autor.codigo}}</td>
                                        <td>{{Autor.nombre}}</td>
